...
def model_evaluate(model, train_dataset, device):
    ...
    similarities = []
    tokenizer = BertTokenizer.from_pretrained('/data/czy/code/MBRL/checkpoint/bert')
    criterion = nn.CrossEntropyLoss()  # 不进行 reduction，保留每个样本的 loss
    dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=32, pin_memory=True)
    ...
    for image, text, label in dataloader:
        ...
        out_mm, out_m1, out_m2, f_img, f_txt = model(image.unsqueeze(2).float(), text)  # 正向传播

        loss = criterion(out_mm, label) + criterion(out_m1, label) + criterion(out_m2, label)
        loss.backward()
        grad_magnitude = sum(param.grad.norm().item() for param in model.parameters() if param.grad is not None)
        grad_magnitudes.append(grad_magnitude)

        out_m1_norm = F.normalize(out_m1, p=2, dim=1)  # 对图像模态输出进行L2归一化
        out_m2_norm = F.normalize(out_m2, p=2, dim=1)  # 对文本模态输出进行L2归一化

        cosine_sim = F.cosine_similarity(out_m1_norm, out_m2_norm, dim=1)
        similarities.append(cosine_sim.item())

    # 归一化梯度幅度和余弦相似度
    grad_magnitudes_norm = normalize_min_max(grad_magnitudes)
    similarities_norm = normalize_min_max(similarities)

    # 结合归一化的余弦相似度和梯度幅度，计算综合难度得分
    combined_scores = [(1 - sim) + grad for sim, grad in zip(similarities_norm, grad_magnitudes_norm)]  # 相似度低 + 梯度大 = 更难

    # 根据综合得分从易到难排序
    sorted_indices = sorted(range(len(combined_scores)), key=lambda k: combined_scores[k], reverse=False)  # 从易到难排序
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/CL_PreSim+GradFD_multiloss_.txt'
