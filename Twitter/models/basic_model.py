...
class CLIP_2(nn.Module):
    def __init__(self, args):
        ...
        self.m1_head = nn.Linear(2048, 3)
        self.m2_head = nn.Linear(2048, 3)
        self.linear_last = nn.Linear(2048 * 2, 3)
    ...
    def forward(self, image, text):
        ...
        txt = self.fc(txt)  # 2048

        out_m1 = self.m1_head(img) # (64,3)
        out_m2 = self.m2_head(txt) # (64,3)
        out_mm = self.linear_last(torch.cat((img, txt), dim=1)) # (64,3)

        # return out_mm, img, txt
        return out_mm, out_m1, out_m2, img, txt
