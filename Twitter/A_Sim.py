...
def model_evaluate(model, train_dataset, device):
    ...
    with torch.no_grad():  # 禁用梯度计算
        for image, text, label in dataloader:
            ...
            # 归一化图像和文本特征
            f_img_norm = F.normalize(f_img, p=2, dim=1)  # 对 f_img 特征进行 L2 归一化
            f_txt_norm = F.normalize(f_txt, p=2, dim=1)  # 对 f_txt 特征进行 L2 归一化

            # 计算归一化后的图像和文本特征的余弦相似度
            cosine_sim = F.cosine_similarity(f_img_norm, f_txt_norm, dim=1)  # 计算每个样本的相似度 (shape: [batch_size])
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/CL_Sim_singleloss.txt'
