...
def model_evaluate(model, train_dataset, device):
    ...
    with torch.no_grad():  # 禁用梯度计算
        for image, text, label in dataloader:
            ...
            # 正向传播，获取图像和文本特征
            out_mm, out_m1, out_m2, f_img, f_txt = model(image.unsqueeze(2).float(), text)
            # out_mm, f_img, f_txt = model(image.unsqueeze(2).float(), text)

            # 归一化图像和文本特征
            out_m1 = F.normalize(out_m1, p=2, dim=1)  # 对 f_img 特征进行 L2 归一化
            out_m2 = F.normalize(out_m2, p=2, dim=1)  # 对 f_txt 特征进行 L2 归一化

            # 计算归一化后的图像和文本特征的余弦相似度
            cosine_sim = F.cosine_similarity(out_m1, out_m2, dim=1)  # 计算每个样本的相似度 (shape: [batch_size])

            # 将相似度添加到列表中
            similarities.extend(cosine_sim.cpu().numpy())  # 将相似度从 GPU 移到 CPU 并转为 NumPy 数组
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/CL_PreSim_multiloss.txt'
