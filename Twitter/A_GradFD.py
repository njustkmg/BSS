...
def model_evaluate(model, train_dataset, device):
    ...
    dataloader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=32, pin_memory=True)  # 使用 batch_size=1，确保逐样本计算梯度
    ...
    for image, text, label in dataloader:
        ...
        model.zero_grad()  # 清除前一个样本的梯度
        out_mm, f_img, f_txt = model(image.unsqueeze(2).float(), text)  # 正向传播
        loss = criterion(out_mm, label)  # 计算每个样本的损失

        # 反向传播计算梯度
        loss.mean().backward()  # 计算每个样本的梯度
    ...
    # 根据梯度幅度从大到小排序
    sorted_indices = sorted(range(len(grad_magnitudes)), key=lambda k: grad_magnitudes[k], reverse=True)  # True表示按从难到易排序
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/CL_GradFD_singleloss.txt'
