...
def model_evaluate(model, train_dataset, device):
    ...
    for image, text, label in dataloader:
        ...
        f_img_norm = F.normalize(f_img, p=2, dim=1)  # 对图像模态输出进行L2归一化
        f_txt_norm = F.normalize(f_txt, p=2, dim=1)  # 对文本模态输出进行L2归一化

        cosine_sim = F.cosine_similarity(f_img_norm, f_txt_norm, dim=1)
    ...
    sorted_indices = sorted(range(len(combined_scores)), key=lambda k: combined_scores[k], reverse=False) # false is CL
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/CL_Sim+GradFD_multiloss.txt'
