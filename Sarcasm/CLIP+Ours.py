...
def train(args, epoch, net, device, train_dataloader, optimizer, scheduler, epoch_step_train):
    ...
    for step, (image, text, label) in enumerate(train_dataloader):
        ...
        text = clip.tokenize(text, truncate=True).to(image.device)
...
def test(args, epoch, net, device, test_dataloader, optimizer, epoch_step_test):
    ...
    for step, (image, text, label) in enumerate(test_dataloader):
        with torch.no_grad():
            ...
            out_mmm = out_m1 + out_m2 + out_mm
            out_m11 = out_m1
            out_m22 = out_m2
            probs_mmm = F.softmax(out_mmm, dim=1)
            probs_m11 = F.softmax(out_m11, dim=1)
            probs_m22 = F.softmax(out_m22, dim=1)
            preds_mmm = (probs_mmm).argmax(dim=1)
            preds_m11 = (probs_m11).argmax(dim=1)
            preds_m22 = (probs_m22).argmax(dim=1)

            all_preds_mmm.extend(preds_mmm.tolist())
            all_preds_m11.extend(preds_m11.tolist())
            all_preds_m22.extend(preds_m22.tolist())
    ...
    # 注意转换为NumPy数组用于性能计算
    all_preds_np_mmm = np.array(all_preds_mmm)
    all_preds_np_m11 = np.array(all_preds_m11)
    all_preds_np_m22 = np.array(all_preds_m22)
    all_labels_np = np.array(all_labels)

    accuracy_mmm = accuracy_score(all_labels_np, all_preds_np_mmm)
    accuracy_m11 = accuracy_score(all_labels_np, all_preds_np_m11)
    accuracy_m22 = accuracy_score(all_labels_np, all_preds_np_m22)
    f1_macro_mmm = f1_score(all_labels_np, all_preds_np_mmm, average='macro')
    f1_macro_m11 = f1_score(all_labels_np, all_preds_np_m11, average='macro')
    f1_macro_m22 = f1_score(all_labels_np, all_preds_np_m22, average='macro')

    return _loss / epoch_step_test, accuracy_mmm, f1_macro_mmm, accuracy_m11, f1_macro_m11, accuracy_m22, f1_macro_m22
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/SPL-clip-only.txt'
        ...
        for epoch in range(args.epochs):
            ...
            mean_loss_test, accuracy_mmm, f1_macro_mmm, accuracy_m11, f1_macro_m11, accuracy_m22, f1_macro_m22 = test(args, epoch, net, device, test_dataloader, optimizer, epoch_step_test)

            print('********************************************************************')
            print('Epoch:' + str(epoch + 1) + '/' + str(args.epochs))
            print('Now train_loss: %.4f || Now test_loss: %.4f' % (mean_loss_train, mean_loss_test))
            print('Now test_acc: %.4f || Now test_f1: %.4f' % (accuracy_mmm, f1_macro_mmm))

            with open(results_file, 'a') as f:
                f.write(f'{epoch + 1} || {accuracy_mmm:.4f} {f1_macro_mmm:.4f} || {accuracy_m11:.4f} {f1_macro_m11:.4f} || {accuracy_m22:.4f} {f1_macro_m22:.4f}\n')

            if accuracy_mmm > best_acc:
                best_acc = float(accuracy_mmm)
