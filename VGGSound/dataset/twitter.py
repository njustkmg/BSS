...
class dataTwitter(Dataset):
    def __init__(self, ann_file, transform, image_root, max_words = 20):
        ...
        # Loading all txt
        total_text_list = []
        ann_file1 = '/data/gzh/MMNAS/data/Twitter15/annotations/train.tsv'
        ann_file2 = '/data/gzh/MMNAS/data/Twitter15/annotations/dev.tsv'
        ann_file3 = '/data/gzh/MMNAS/data/Twitter15/annotations/test.tsv'
        info1_ = pd.read_csv(ann_file1, sep='\t')
        info2_ = pd.read_csv(ann_file2, sep='\t')
        info3_ = pd.read_csv(ann_file3, sep='\t')
        info1 = info1_['String']
        info2 = info2_['#3 String']
        info3 = info3_['String']
    ...
    def __getitem__(self, index):
        ...
        # faster r-cnn
        # imagePath_tmp = self.image_root + self.info["ImageID"][index]
        # _, filename = os.path.split(imagePath_tmp)
        # name, _ = os.path.splitext(filename)
        # imagePath = './data/Twitter15/TW_farcn_feat/'+name+'.npy'
        # image_ = np.load(imagePath)
