...
def model_evaluate(net, train_dataset, device):
    ...
    for step, bag in enumerate(dataloader):
        ...
        # 计算单模态分类头的余弦相似度
        # out_m1_norm = F.softmax(out_m1, p=2, dim=1)
        # out_m2_norm = F.softmax(out_m2, p=2, dim=1)
    ...
    # sorted_indices = sorted(range(len(combined_scores)), key=lambda k: combined_scores[k], reverse=True) # False是CL
...
if __name__ == '__main__':
    ...
    if True:
        ...
        results_file = './results/CL_PreSim+Loss.txt'
